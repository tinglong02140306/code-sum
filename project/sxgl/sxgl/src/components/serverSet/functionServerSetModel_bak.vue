 ...
 * @Last Modified time: 2019-01-03 21:25:31
 */
<template>
            ...
            <div class="right-wrap fr">
                ...
                <div class="list-wrap" v-show="!shouNoData">
                    <div class="server-info-wrap">
                        <p class="server-info-title font-min">
                            ...
                            <div class="sub-wrap clearfix" :style="{width: subWrapWid}">
                                <div class="sub-item" :style="{width: subWid}">
                                    <span class="switch-default clearfix" v-for="(views ,index) in infoDetail.outputList" :class="{cur:iscur==index}" :title="views.templateName">
                                        <span ref="navs" @click="navClick(index)" class="ell font-max">{{views.tableName}}</span>
                                        <i @click="delTree(index)" class="iconfont icon-fork"></i>
                                    </span>
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        //删除树复选框选中的数据
        delTree(index) {
            this.$refs.inputTable.toggleRowSelection(this.infoDetail.inputList[index], false);
            this.infoDetail.inputList[index].bindId = '';
        },
        //向左滚动
        scrollLast() {
            this.distance -= 50;
        ...
        inputChangeFun(val) {
            ...
            if (that.param.isMult == 0) {
                if (that.flag == 1 && val.length != 0) {  //清空所有选中的复选框
                    for (let i = 0; i < that.infoDetail.inputList.length; i++) {
                        if (that.infoDetail.inputList[i].paramCode == val[val.length - 1].paramCode) {
                            that.inputSelectIndex = i;
                        }
                    }
                    that.flag = 2;
                    that.$refs.inputTable.clearSelection();
                } else if (that.flag == 2) { //选中最后一次的复选框
                    that.flag = 3;
                    that.$refs.inputTable.toggleRowSelection(that.infoDetail.inputList[that.inputSelectIndex], true)
                } else if (that.flag == 3) { //获取数据
                    that.flag = 1;
                    val[0].tempAttrId = that.param.tempAttrId;
                    arr.push(val[0]);
                ...
                if (res.flag == true) {
                    ...
                    for (let i = 0; i < res.data.length; i++) {  //循环赋值，主要是保持用户选中的状态不被刷新
                        for (let j = 0; j < _that.infoDetail.outputList.length; j++) {
                            if (res.data[i].id == _that.infoDetail.outputList[j].id && res.data[i].inputParamCode == _that.infoDetail.outputList[j].inputParamCode) {
                    ...
                    setTimeout(() => {
                        ...
                        for (let i = 0; i < _that.infoDetail.outputList.length; i++) {
                            for (let j = 0; j < _that.infoDetail.outputList[i].outputList.length; j++) {
                                if (_that.infoDetail.outputList[i].outputList[j].bindId) {
        ...
        outputChangeFun(val, index) {
            for (let i = 0; i < this.infoDetail.outputList.length; i++) {
                this.outputChangeId[index] = JSON.parse(JSON.stringify(val));
            }
        },
        //展开树结构
        showTree() {
        ...
        */
        ...
        saveDataHandle() {
            ...
            if (outputChangeId.length != 0) {
                for (let i = 0; i < outputChangeId.length; i++) {
                    if (outputChangeId[i].length == 0) {
                        this.$Message.warning('请选择一个返回参数');
                        this.$refs.navs[i].click();
                        return [];
                    }
                }
            }
        ...
        getYsmcData() {
            ...
            unit.ajaxMerPost('/znsj-web/data/temp/attr/getAttrDictByTempId', jsonObj, function (res) {
                if (res.flag == true) {
