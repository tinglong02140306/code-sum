...
class BillEtcStore {
    ...
    //列表查询
    @observable dataList = [];
    @action getOrderEtcQuery(params) {
        this.params = params;
        if (!this.isShowLoading) {
            this.setIsShowLoading(true);
        }
        console.log(JSON.stringify(params));
        http.post('/website/bill-fix/order-query-etc',params,response=>{
    ...
    //差调
    @action billFixEtc(order_no,flag){
        const params = {
            order_no:order_no,
            flag:flag
        }
        this.setIsShowLoading(true);
        http.post('/website/bill-fix/fix-etc',params,response=>{
            this.setIsShowLoading(false);
            message.info("操作成功");
            this.getOrderEtcQuery(this.params);
        },err=>{
            this.setIsShowLoading(false);
            message.error(err);
        });
