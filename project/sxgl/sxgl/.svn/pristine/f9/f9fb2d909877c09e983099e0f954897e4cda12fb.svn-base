      ...
      <el-dialog :title="modelTitle" :visible.sync="addModal" width="50%" center>
        <el-form :model="formItem" :rules="ruleValidate" ref="formValidate" label-width="100px">
          <el-form-item label="分类名称" prop="classifyName">
            <el-input v-model="formItem.classifyName" maxLength=50 autocomplete="off" @change="(val)=>changeInput1(val,'classifyName')" ></el-input>
          </el-form-item>
          ...
          <el-form-item label="说明" prop="detail">
            <el-input type="textarea" maxLength=50 v-model="formItem.detail" @change="(val)=>changeInput1(val,'detail')" @keyup.enter="getInputEvt" @input="getInputEvt($event)"></el-input>
          </el-form-item>
        </el-form>
      ...
      <el-dialog :title="modelTitleChild" :visible.sync="addModalChild" width="50%" center>
        <el-form :model="formItemChild" :rules="ruleValidate" ref="formValidateChild" label-width="100px">
          <el-form-item label="答案名称" prop="answerName">
            <el-input v-model="formItemChild.answerName" maxLength=50 autocomplete="off" @change="(val)=>changeInput2(val,'answerName')"></el-input>
          </el-form-item>
          ...
          <el-form-item label="说明" prop="detail">
            <el-input type="textarea" maxLength=50 v-model="formItemChild.detail" @change="(val)=>changeInput2(val,'detail')" @keyup.enter="getChildInputEvt" @input="getChildInputEvt($event)"></el-input>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button type="primary" size="small" @click="childSubmitForm('formValidateChild', modelTitleChild)">确 定</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeInput1(val,attr){
        this[formItem][attr]=val==''?'':val;
      },
      changeInput2(val,attr){
        this[formItemChild][attr]=val==''?'':val;
      },
      /**
       *  修复ie记忆问题
       *  批量新增答案
        ...
        if(row.status == '1') {
          ...
          if (row.isNew == '1') {
                addDto = {
                    ...
                    //事项编码
                    objectCode: that.matterCode,
                    situationMattCode: that.matterCode,
                    objectType: '1',//1:事项 2:一件事
        ...
        let that = this;
          if(!that.notSaveFlag) {
              that.notSaveChildData = [{
                ...
                classifyName: that.curClassifyName,
                answerName: '',
                defaultSelect: '0',
                detail: '',
                status: '0',
      ...
      addEvtChild(index, row) {
        ...
        that.$nextTick(() => {
            ...
            that.curClassifyName = row.classifyName;
      ...
      typeSave(url, data, option) {
        ...
        util.ajaxObjPost(url, data, function (res) {
          if (res.flag) {
            ...
            self.$Message.warning(res.errMsg);
          }
        }, function (res) {
