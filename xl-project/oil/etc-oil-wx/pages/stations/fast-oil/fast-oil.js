...
Page({
  ...
  onInputMoney:function(e){
    ...
    if(value&&!checkMoney(value)){
      ...
      if (value){
        ...
        this.setData({discount_amount:0});
      }
    }
  },
  ...
  onSubmitClick:function(){
    const {oil_info, inputMoney,isCanSubmit} = this.data;
    if(isCanSubmit){
      if(inputMoney&&checkMoney(inputMoney)){
        ...
        wx.showToast({
          title:inputMoney?"请输入正确的金额":"请输入加油金额",
          icon:"none"
        });
      }
    }
  ...
  submitOrder:function(params){
    ...
    getPostPromise(stationsApi.order,params).then(res=>{
      ...
      const details = encodeURIComponent(JSON.stringify(params));
      wx.navigateTo({url:`/pages/stations/fast-order/fast-order?details=${details}`})
    }).catch(err=>{
      wx.showToast({
  ...
  getDiscount:function(params){
    getPostPromise(stationsApi.discount,params).then(res=>{
      ...
      this.setData({discount_amount:0,isCanSubmit:false});
