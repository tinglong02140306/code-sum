<template>
    <div class="m-wrap">
        ...
        <div class="select-wrap hide">
            <p v-for="(item ,index) in listData" :class="{select: item.blankForm}" @click="jump(item)">{{index + 1}}、{{item.matterialName}}</p>
        </div>
    </div>
</template>
<style>
...
export default {
    ...
    mounted() {
        ...
        //     this.listData[i].url = this.listData[i][this.type];
        // }
    },
    methods: {
        handleSelect(e) {
        ...
        jump(data) {
            if (data.blankForm) {
                util.openFullWindow(data.blankForm);
            }
        }
    }
}
