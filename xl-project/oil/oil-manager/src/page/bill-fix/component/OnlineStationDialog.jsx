...
@observer
class OnlineStationDialog extends Component {
    ...
    componentDidMount() {
        this.props.BillOnlineStore.getCompanyQuery();
        ...
        this.props.BillOnlineStore.getStationQuery(params);
    }
    ...
    handleTableChange = (pagination) => {
        ...
        this.props.BillOnlineStore.setSearchPagination(pager);
    ...
    selectCompanyOption=()=>{
        const {companyList} = this.props.BillOnlineStore;
    ...
    onCancel=()=>{
        this.props.BillOnlineStore.setIsShowSearchDialog(false);
    }

    onOk=()=>{
        const {stationList,typeModal} = this.props.BillOnlineStore;
        const {selectedRowKeys} = this.state;
        const key = selectedRowKeys[0];
        if (typeModal === 3){
            this.props.BillOnlineStore.setSearchData(stationList[key]);
        } else {
            this.props.BillOnlineStore.setOperationData(stationList[key]);
        }
        this.props.BillOnlineStore.setIsShowSearchDialog(false);
    }

    render() {
        const {isShowSearchLoading,searchPagination,stationList,isShowSearchDialog} = this.props.BillOnlineStore;
...
export default OnlineStationDialog;
