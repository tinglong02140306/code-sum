 ...
 * @Last Modified time: 2018-10-26 01:31:03
 */
...
export default {
    ...
    methods: {
        ...
        getCode(val) {
            let pEle = $('.switch-navigate p:last-child');
            pEle.css({'cursor':'pointer'});
    ...
    mounted() {
        ...
        pEle = $('.switch-navigate p:last-child');
        switch (type) {
            case 'addNew':
        ...
        if(type === 'addNew') {
            pEle.css({'cursor':'not-allowed'});//配置信息tab禁用效果
        } 
