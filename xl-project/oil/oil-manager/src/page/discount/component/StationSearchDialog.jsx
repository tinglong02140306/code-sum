...
class StationSearchDialog extends Component {
    ...
    componentDidMount() {
        this.props.discountStationStore.getCompanyQuery();
        ...
        this.props.discountStationStore.getStationQuery(params);
    }
    ...
    handleTableChange = (pagination) => {
        ...
        this.props.discountStationStore.setSearchPagination(pager);
    ...
    selectCompanyOption=()=>{
        const {companyList} = this.props.discountStationStore;
    ...
    onCancel=()=>{
        this.props.discountStationStore.setIsShowSearchDialog(false);
    }

    onOk=()=>{
        const {stationList,typeModal} = this.props.discountStationStore;
            const {selectedRowKeys} = this.state;
            const key = selectedRowKeys[0];
            if (typeModal === 3){
                this.props.discountStationStore.setSearchData(stationList[key]);
        } else {
            this.props.discountStationStore.setOperationData(stationList[key]);
        }
        this.props.discountStationStore.setIsShowSearchDialog(false);
    }

    render() {
        const {isShowSearchLoading,searchPagination,stationList,isShowSearchDialog} = this.props.discountStationStore;
