...
class BillPartnerStore {
    ...
    @observable recodeObject = false;
    @action setRecodeObject(recodeObject) {
        this.recodeObject = recodeObject;
    }
    ...
    //列表查询
    @observable dataList = [];
    @action getOrderPartnerQuery(params) {
        this.params = params;
        if (!this.isShowLoading) {
            this.setIsShowLoading(true);
        }
        console.log(JSON.stringify(params));
        http.post('/website/bill-fix/order-query-partner',params,response=>{
    ...
    //差调
    @action billFixPartner(order_no,flag,out_user_id,callBack){
        const params = {
            order_no:order_no,
            flag:flag,
        ...
        this.setIsShowLoading(true);
        http.post('/website/bill-fix/fix-partner',params,response=>{
            this.setIsShowLoading(false);
            message.info("操作成功");
            this.getOrderPartnerQuery(this.params);
            callBack();
        },err=>{
            this.setIsShowLoading(false);
            message.error(err);
...
export default BillPartnerStore;