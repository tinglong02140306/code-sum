...
@inject("BillEtcStore")
@observer
class EtcStationDialog extends Component {
    ...
    componentDidMount() {
        this.props.BillEtcStore.getCompanyQuery();
        ...
        this.props.BillEtcStore.getStationQuery(params);
    }
    ...
    handleTableChange = (pagination) => {
        ...
        this.props.BillEtcStore.setSearchPagination(pager);
    ...
    selectCompanyOption=()=>{
        const {companyList} = this.props.BillEtcStore;
    ...
    onCancel=()=>{
        this.props.BillEtcStore.setIsShowSearchDialog(false);
    }

    onOk=()=>{
        const {stationList,typeModal} = this.props.BillEtcStore;
        const {selectedRowKeys} = this.state;
        const key = selectedRowKeys[0];
        if (typeModal === 3){
            this.props.BillEtcStore.setSearchData(stationList[key]);
        } else {
            this.props.BillEtcStore.setOperationData(stationList[key]);
        }
        this.props.BillEtcStore.setIsShowSearchDialog(false);
    }

    render() {
        const {isShowSearchLoading,searchPagination,stationList,isShowSearchDialog} = this.props.BillEtcStore;
...
export default EtcStationDialog;
