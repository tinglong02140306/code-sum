...
import { Table, Button, Popconfirm, Form, Input, Icon, message, Select, Modal, TimePicker, InputNumber, Radio, Cascader } from 'antd';
import { inject, observer } from 'mobx-react/index';
import Upload from '../../../component/upload/Upload';
// import { options } from '../../../constants/city';
...
class Info extends React.Component {
    ...
    onOptionsClick = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            console.log(values);
            if (!err) {
                const { options_obj, options_type, province, provinceCode, city, cityCode, county, countyCode } = this.state;
                let areaObj = {};
                ...
                if (options_type == 1 || options_type == 3) {
                    areaObj = {
                        province,
                        provinceCode,
                        city,
    ...
    exportDetail = () => {
        ...
        this.props.vehicleInfoStore.exportDetail(params);
    };
    // 上下线
    offOnLine = (record) => {
        this.props.vehicleInfoStore.offOnLine({ ids: [record.id], deleted: !record.deleted }, () => {
            this.fetch();
        });
    }
    // 所属区域选择
    onCascaderChange = (value, code) => {
        this.setState({
            province: value[0],
            provinceCode: code[0].code,
            city: value[1],
            cityCode: code[1].code,
            county: value[2],
            countyCode: code[2].code
        })
    };
    dealAreaData = (options_type, options_obj) => {
        if (options_type == 1 || options_type == 2) {
            const { province, province_code, city, city_code, county, county_code } = options_obj;
            // 区域处理
            options_obj.areaName = [province, city, county];
            this.setState({
                province: province,
                ...
                <Form className="vehicle-info-modal-form" layout="inline" onSubmit={this.onOptionsClick}>
                    <div className="vehicle-info-modal-form-box">
                        <div className="vehicle-info-modal-form-box-side">
                            ...
                            <Form.Item className="vehicle-info-modal-form-item" label="所属区域">
                                {getFieldDecorator('area', {
                                    initialValue: options_obj && options_obj.areaName,
                            ...
                            <Form.Item className="vehicle-info-modal-form-item" label="能否停止">
                                {getFieldDecorator('allowStop', { rules: [{ required: true, message: '请选择能否停止' }], initialValue: options_obj ? options_obj.allow_stop : true })
                            ...
                            <Form.Item className="vehicle-info-modal-form-item" label="结算归属">
                                {getFieldDecorator('belong', { initialValue: options_obj && options_obj.belong })(
                                    <Select placeholder="请选择结算归属" disabled={disabled}>
                                        ...
                                        <Select.Option value={3}>美能达</Select.Option>
                                        <Select.Option value={4}>大盒子</Select.Option>
                                    </Select>
                                )}
                            </Form.Item>
    ...
    header = () => {
        return (
            <div>
                ...
                <Button size="small" type="primary" style={{ marginLeft: 10 }} onClick={() => this.exportDetail()}>
                    导出明细
				</Button>
            </div>
        );
                ...
                <Form className="vehicle-info-search-container" layout="inline" onSubmit={this.onQueryClick}>
                    ...
                    <Form.Item className="vehicle-info-search-item" label="洗车机品牌">
                        {getFieldDecorator('brand_id', { initialValue: "" })
                            (<Select size="small" placeholder="请选择洗车机品牌" >
    ...
    columns = [
        ...
        // }, 
        {
            ...
            align: 'center',
            render: (record) => {
                const belong = record === 1 ? "信联" : record === 2 ? "小睿" : record === 3 ? "美能达" : record === 4 ? "大盒子" : "--"
        ...
        // {/* false 上线  true 下线 */}
        {
            ...
            align: 'center',
            render: (record) => {
                return <div>{record ? '下线' : '上线'}</div>;
            }
        },
        {
            title: '能否停止',
            dataIndex: 'allow_stop',
            key: 'allow_stop',
            align: 'center',
            render: (record) => {
                return <div>{record ? '是' : '否'}</div>;
            }
        },
        {
            title: '洗车机编号',
                ...
                return (
                    <div className="vehicle-info-options-box">
                        <div
                            ...
                            onClick={() => { this.dealAreaData(2, record); }}
                        >
                            <Icon type="eye" />查看
						</div>
                        <div
                            className="vehicle-info-options-item update"
                            onClick={() => { this.dealAreaData(1, record); this.setState({ options_type: 1, options_obj: record }) }}
                        >
                            <Icon type="edit" />修改
						</div>

                        <div
                            className="vehicle-info-options-item onoff"
                            style={{ color: record.deleted ? 'rgb(183, 181, 181)' : 'rgb(255, 85, 1)' }}
                            onClick={() => this.offOnLine(record)}>
                            {/* false 上线  true 下线 */}
                            <Icon type="appstore" />{record.deleted ? "上线" : "下线"}
                        </div>
                    </div>
                );
            }
