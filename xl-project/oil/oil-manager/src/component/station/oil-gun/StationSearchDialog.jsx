...
import './StationSearchDialog.scss';
...
class StationSearchDialog extends Component {
    ...
    componentDidMount() {
        this.props.stationGunStore.getCompanyQuery();
        ...
        this.props.stationGunStore.getStationQuery(params);
    }
    ...
    handleTableChange = (pagination) => {
        ...
        this.props.stationGunStore.setSearchPagination(pager);
    ...
    selectCompanyOption=()=>{
        const {companyList} = this.props.stationGunStore;
    ...
    onCancel=()=>{
        this.props.stationGunStore.setIsShowSearchDialog(false);
    };

    onOk=()=>{

        const {stationList,typeModal} = this.props.stationGunStore;
        const {selectedRowKeys} = this.state;
        const key = selectedRowKeys[0];
        if (typeModal === 3){
            this.props.stationGunStore.setSearchData(stationList[key]);
            const params = {
                page_num:1,
                page_size:10,
                station_id: isEmpty(stationList[key])?null:stationList[key].id
            }
            this.props.stationGunStore.getOilGunQuery(params);
        } else {
            this.props.stationGunStore.setOperationData(stationList[key]);
        }
        this.props.stationGunStore.setIsShowSearchDialog(false);
    };
    ...
    render() {
        const {isShowSearchLoading,searchPagination,stationList,isShowSearchDialog} = this.props.stationGunStore;
