    ...
    methods: {
        ...
        xzqhLoadData(item, callback) {
            ...
            setTimeout(() => {
                ...
                unit.ajaxMerPost('/znsj-web/commer/getXzqhTreeData', obj, function (result) {
                    ...
                    // else  {
                    //     that.xzqhVal = item.__value.split(',');
                    // }
        ...
        */
        ...
        applyerChange(data) {
            let that = this;
            if (data == '1') { // 个人
                that.singPerActive = false;
                that.legalPerActive = true;
            } else if (data == '2') {
                that.singPerActive = true;
                that.legalPerActive = false;
            }
        },
        /*
         *  送达方式 change事件监听
        ...
        smsTypeChange(data) {
            let that = this;
            // 3 // 快递寄送
            that.smsFormActive = data == '03' ? false : true;
        ...
        preEvent() {
            let that = this;
            that.$emit('go', 'materialVerification', {
                flag: true
            });
        },
        /*
        ...
        nextEvent() {
            ...
            // 请求接口参数格式处理
            let Urlparam = that.$parent.param,
            ...
            obj = $.extend({}, obj, {
                id: Urlparam.matterIds.join(','),
                matterInfos: codeVerArr,
                // receiptNo: Urlparam.receiptNo,  // 收件编号
                type: Urlparam.type,  // 收件类型 1：单事项收件 2：多事项收件 3：一件事收件 that.$parent.param.type
            });
            if(that.pageParamObj.type == '3') {  // 事件
                obj.receiptNo = Urlparam.eventReceiptNo;
            }else {
                obj.receiptNo = Urlparam.receiptNo;
            }
        ...
        init() {
            ...
            that.prBtn = ($('.half-row').width() - (140 + $('.ivu-input-wrapper').width())) + 'px';
            that.xzqhWidth = ($('.row').width() / 2 -25) + 'px';
        }
