 ...
 * @Last Modified time: 2018-12-10 21:08:13
* @description 导览图
*/
<template>
  ...
  methods: {
    setWidthAndReset(treeData, index) {
      ...
      for (var i = 0; i < this.treeData.children.length; i++) {
        ...
        if (this.treeData.children[i].pzType == '01' || this.treeData.children[i].isDefaultHight) { //pzType：01必须提供  02 视情形提供
          ...
          this.defaultSelectMatter(this.treeData.children[i], index);
        }
        g.setNode(this.treeData.children[i].id, value);
      };
      // 设置路线关系
      for (var i = 0; i < this.treeData.children.length; i++) {
        var item = this.treeData.children[i];
        if (item && item.childList) {
          for (var j = 0; j < item.childList.length; j++) {
            g.setEdge(item.id, item.childList[j], {
              arrowhead: 'vee',//箭头类型
      ...
      if (!this.isIE9) {
        ...
        //设置svg高度
        // this.svg.attr("height", g.graph().height * initialScale + 10);
        //设置svg高度
    ...
    highlightNode(id) {
      if (this.svg.selectAll) {
        for (var i = 0; i < this.svg.selectAll("g.node")._groups[0].length; i++) {
          var node = this.svg.selectAll("g.node")._groups[0][i];
          if (node.__data__ == id) {
