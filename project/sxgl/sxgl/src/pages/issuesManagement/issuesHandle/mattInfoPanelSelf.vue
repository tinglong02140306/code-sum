 ...
 * @Last Modified time: 2019-02-13 20:50:23
 */
...
<template>
    ...
    <div id="mattInfoPanelSelf">
        ...
        <el-form :model="ruleForm" :rules="rules" ref="mattForm" label-width="123px" class="demo-ruleForm" size="small">
                    ...
                    <el-row>
                        <el-col :span="24">
                            <el-form-item label="事项名称" prop="matterName">
                                ...
                                <el-input v-model="ruleForm.matterName" maxlength="100" placeholder="请输入事项名称" :disabled="isDisabled" @focus="matterNameChangeEvt"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                ...
                <el-col :span="9" :push="3">
                    <el-col :span="24">
                        <el-form-item label="缩略图" prop="img" class="thumbnail">
                            ...
                            <vue-upload-web ref="upload" :url="ruleForm.uploadObj.cdnUrl" :form-data="ruleForm.uploadObj.cdnParams" :accept="ruleForm.uploadObj.accept" :key-generator="keyGenerator" @progress="uploadProgress" @success="uploadSuccessFJ" @before="beforeUpload" @error="uploadError" @complete="handleComplete" upload-button=".upload-btn-one" :multiple=false :fileSingleSizeLimit=208400>
                            </vue-upload-web>
            ...
            <el-row>
                ...
                <el-col :span="9" :push="3">
                    <el-form-item label="所属部门" prop="deptCode">
                        <el-select v-model="ruleForm.deptCode" filterable :disabled="isDisabled" class="xzqhWt" :popper-append-to-body="false">
                            <el-option v-for="(item ,key) in deptData" :key="key" :label="item.label" :value="item.value">
                            </el-option>
                        </el-select>
                    </el-form-item>
                </el-col>
            ...
            <el-row>
                ...
                <el-col :span="9" :push="1">
                    <el-form-item label="事项类型" prop="matterType">
                        <el-select v-model="ruleForm.matterType" filterable :disabled="isDisabled" class="xzqhWt" :popper-append-to-body="false">
                ...
                <el-col :span="20">
                    ...
                    <div id="tipId">
                        <el-row class="mt10 person-select">
                            <el-col :span="9" :push="2" class="ml-10">
                                <el-form-item label="个人主题" prop="personalTheme">
                                    <el-select v-model="ruleForm.personalTheme" multiple filterable :disabled="isDisabledPersonal" class="xzqhWt" :popper-append-to-body="false">
                                        <el-option v-for="item in perThemeData" :key="item.value" :label="item.label" :value="item.value">
                                        </el-option>
                                    </el-select>
                                </el-form-item>
                            </el-col>
                            <el-col :span="9" :push="6" class="ml-10">
                                <el-form-item label="个人对象" prop="personalObject">
                                    <el-select v-model="ruleForm.personalObject" multiple filterable :disabled="isDisabledPersonal" class="xzqhWt" :popper-append-to-body="false">
                        ...
                        <el-row class="mt10 legal-select">
                            <el-col :span="9" :push="2" class="ml-10">
                                <el-form-item label="法人主题" prop="legalTheme">
                                    <el-select v-model="ruleForm.legalTheme" multiple filterable :disabled="isDisabledLegal" class="xzqhWt" :popper-append-to-body="false">
                            ...
                            <el-col :span="9" :push="6" class="ml-10">
                                <el-form-item label="法人对象" prop="legalObject">
                                    <el-select v-model="ruleForm.legalObject" multiple filterable :disabled="isDisabledLegal" class="xzqhWt" :popper-append-to-body="false">
            ...
            <!-- 选择事项弹窗 -->
            <el-dialog title="请选择事项" :visible.sync="addCaseDialog" width="70%">
                <!-- @refreshList="getConfigData" -->
                <matt-case ref='addCaseDialog' type="selection" @closeDialog="closeDialog" @matterValueChangEvt="matterValueChangEvt"></matt-case> 
            </el-dialog>
           <el-row>
               <el-col :span="20" :push="1">
                    <div v-if="btnVisiable" class="mt10 tr">
...
import addMattCase from '@/pages/issuesManagement/issuesHandle/addMattCase';
...
export default {
    ...
    methods: {
        matterValueChangEvt(val) {
            this.ruleForm.matterName = val;
            this.closeDialog();
        },
        matterNameChangeEvt() {
            let that = this;
            if(this.$parent.sourceForm.dataSource == '01') {
                that.addCaseDialog = true;
            }
        },
        closeDialog() {
        ...
        */
        submitForm(formName) {
            let _that = this,
                type = _that.saveType,
                url;
            _that.ruleForm.resultFile = this.$refs.extendInfo.ruleForm.resultFile;
        ...
        getEditDefaultXzqh() {
            ...
            unit.ajaxMerPost('/znsj-web/matterAllocation/getInfo', {
                ...
                if (res.flag) {
                    ...
                    // that.$Message.warning('服务端错误21');
                }
            }, function (res) {
                // that.$Message.warning('服务端错误22');
            }, that);
    ...
    activated() {
        let _that = this;
        if (_that.ueditorContent) {
            setTimeout(function () {
                _that.$refs.ue.setContent(_that.ueditorContent);
            }, 500);
        }
    },
    deactivated() {
        this.ueditorContent = this.$refs.ue.getUEContent();
    }
};
</script>
