...
var baiduPostionTimer,  // 百度定位次数
   ...
   /* pointDataArrs = [
	...
	eventHandle = {
		...
		// 根据点的数组自动调整缩放级别
        setMapZoom: function(pointDataArrs) {
            var view = mapItem.getViewport(eval(pointDataArrs)),
                mapZoom = view.zoom,
        ...
        renderMap: function(){
            ...
            if(pointDataArr.length >= 2 && i <= 4) {
                var strackline = GDouglasPeucker(pointDataArr,70); //点集合抽稀，第二个参数，抽稀线路偏离角度，值越大，越平滑
                ...
                for(var j = 0; j  < bezier.geometry.coordinates.length; j++) {
                    pointDataArrs.push(new BMap.Point(bezier.geometry.coordinates[j][0], bezier.geometry.coordinates[j][1]));
                }
            }else {
                pointDataArrs.push(new BMap.Point(pointDataArr[0][0], pointDataArr[0][1]));
            }   

            var mPoint = pointDataArrs[pointDataArrs.length -1]; // 创建点坐标
            eventHandle.setMapZoom(pointDataArrs);  // 根据距离设置地图缩放级别
            ...
            eventHandle.setMapConStyle(pointDataArrs);   
        },
        setMapConStyle: function(pointDataArrs) {
            ...
            var polyline =new BMap.Polyline(pointDataArrs, {
