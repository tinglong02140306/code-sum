...
class StationSearchDialog extends Component {
    ...
    componentDidMount() {
        this.props.stationPriceStore.getCompanyQuery();
        ...
        this.props.stationPriceStore.getStationQuery(params);
    }
    ...
    handleTableChange = (pagination) => {
        ...
        this.props.stationPriceStore.setSearchPagination(pager);
    ...
    selectCompanyOption=()=>{
        const {companyList} = this.props.stationPriceStore;
    ...
    onCancel=()=>{
        this.props.stationPriceStore.setIsShowSearchDialog(false);
    };

    onOk=()=>{

        const {stationList,typeModal} = this.props.stationPriceStore;
        const {selectedRowKeys} = this.state;
        const key = selectedRowKeys[0];
        if (typeModal === 3){
            this.props.stationPriceStore.setSearchData(stationList[key]);
            const params = {
                page_num:1,
                page_size:10,
                station_pk: isEmpty(stationList[key])?null:stationList[key].id
            }
            this.props.stationPriceStore.getOilPriceQuery(params);
        } else {
            this.props.stationPriceStore.setOperationData(stationList[key]);
        }
        this.props.stationPriceStore.setIsExecute(true);
        this.props.stationPriceStore.setIsShowSearchDialog(false);
    };


    render() {
        const {isShowSearchLoading,searchPagination,stationList,isShowSearchDialog} = this.props.stationPriceStore;
                ...
                <div className="search-info-container">
                    <div className="search-info-top-container">
                        <Form className="search-info-form" layout="inline">
                            ...
                            <Form.Item label="编号" className="search-info-form-item">
                                <Input size="small"
                                       value={station_id}
                                       style={{width: 100, margin: 0}}
                                       maxLength={20}
                            ...
                            <Form.Item label="合作伙伴" className="search-info-form-item">
                            ...
                            <Form.Item label="使用状态" className="search-info-form-item">
                                <Select className="search-info-item-input"
                                        size="small"
                                        value={usage_status}
                                        style={{width: 100, margin: 0}}
