...
(function(doc, win) {
	...
	if ($.device.android) {
		...
		if (version > 4.4 && ua.indexOf('iflytek_mmp') != -1) {
			// 设置viewport，进行缩放，达到高清效果
			// metaEl.setAttribute('content', 'width=' + screen.width + ',initial-scale=' + scale + ',maximum-scale=' + scale + ',minimum-scale=' + scale + ',user-scalable=no' + ',target-densitydpi=device-dpi' + ',user-scaled=no');
