...
Page({
  ...
  onLoad: function (options) {
    try {
      ...
      const {order_no,etc_card_no,invoice_title,invoice_detail,invoice_total_money,invoice_create_time_str_s,image_path,accept_mobile,
        buyer_address,bank_name,bank_account,allow_red,accept_email,bill_order_no} = details;
      const redaction = {
  ...
  onRedactionClick:function(){
    // const {invoice_total_money,etc_card_no,order_no,invoice_detail,bill_order_no} = this.data;
    // const params = {
    //   from:2,
    //   invoice_total_money:invoice_total_money,
    //   etc_card_no:etc_card_no,
    ...
    // });
    const {allow_red} = this.data;//判断是否已重开过发票
    this.setData({
