...
import xlsxUtils
    from "../../utils/xlsx.utils.min";
import {message} from "antd/lib/index";
import {isEmpty} from "../../utils/isEmpty";

class OilCardStatistics {
    ...
    //计算
    @action getCalculate(start_date, end_date) {
        this.setIsShowOilLoading(true);
        this.cardStatisticsList = [];
        this.product_data = [];
        this.business_type_array = [];//业务类型（首列标题）
        ...
        http.post("/website/report/electronic-card-accountable-amount-calculation", params, response => {
            this.setIsShowOilLoading(false);
            const pagination = {};
            pagination.pageSize = 99;
            pagination.hideOnSinglePage = true;
    ...
    @action downloadExl(start_date, end_date) {
        ...
        let head3 = {[[`A${row}`]]: {"v": "分管领导：    负责人：    复核人：    制表人：    制表日期：  "},};
        mergesArr.push({"s": {"c": 0, "r": row-2}, "e": {"c": 1, "r": row-2}});
        mergesArr.push({"s": {"c": 0, "r": row-1}, "e": {"c": 8, "r": row-1}});
        ...
        const keyMap = [
            ...
            "consume_amount",
            "fee",
            "accounts_payable",];//通过设置数组让导出时可以按顺序显示
        ...
        if (this.product_data.length > 0) {
            ...
            saveAs(URL.createObjectURL(blob), "加油业务电子卡运营统计报表.xlsx");
        } else {
    ...
    "data": [
        ...
        },
        {
            "business_data": [
                ...
                {
                    "accounts_payable": 7400,
                    "amount": 7000,
                    "consume_amount": 7700,
                    "discount": 700,
                    "fee": 300,
                    "product_name": "APP加油-E高速",
                    "product_type": "301010003",
                    "service_charge": 1400,
                    "subtotal_income": 8400
                },
