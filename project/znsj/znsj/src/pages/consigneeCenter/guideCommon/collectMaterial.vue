 ...
 * @Last Modified time: 2019-3-19 15:22:32
 * @description 收材料
 */
<template>
...
export default {
    ...
    methods: {
       ...
       /*
            ...
            if(stateObj.gpySelPicList.length > 0) {
                unit.ajaxObjPost('/znsj-sj-web/commer/uploadbase64', JSON.stringify(stateObj.gpySelPicList), function (res) {
                    ...
                    for(let i = 0; i < data.length; i++) {
                        ...
                        arr.elecUrlListLoc.push(obj);
                        arr.elecUrlList.push(obj);
                    }
                    that.resetGpyData();
                }, function (res) {
        ...
        /*
         * 判断材料的形式 type 02:原件 03：复印件 01：电子件
         */
        judgeForm(type,str) {
            let flag = false;
        ...
        fileCkeckEvt(pos,loc,type,state) {
            let that = this,
                ...
                selectData = selectAllObj.matterList[loc], // 某一事项中的所有材料
        ...
        oriCopyModal(pos,loc,type) {
            ...
            if(type == 'origin') {
                ...
                that.opinion = list.copiesView || '复印件已核';
        ...
        beforeUpload(file) {
            if(file.size > 50485760) {
                this.$Message.warning('上传文件大小不能50M!');
                return;
            }
        },
        /*
        ...
        cerLibSelOk() {
            ...
            for(let i in that.cerLibList) {
                if(that.cerLibList[i].selected === '1') {
                    ...
                    matList.elecUrlListLoc.push(obj);
                    matList.elecUrlList.push(obj);
                }
            }
            if(selectedNum == 0) {
         ...
         */
        cerLibSeCancel() {
            this.photoDiaActive = false;
            // 复选框状态都置为 未选中状态
            for(let i in this.cerLibList) {
                this.cerLibList[i].selected = '0';
            }
        },
        /*
         *  上一步按钮点击事件
        ...
        sureAccFileEvt(type) {
            ...
            obj.id = obj.type == '3' ? param.eventReceiptNo : param.receiptNo;
            obj.matterList =  that.matterList;
            that.pageParamObj.jointStatus = param.jointStatus;
            that.pageParamObj.state = param.state;
            that.loadingFun();
        ...
        init() {
            let that = this,
            param = that.$parent.param;
            that.pageParamObj.type = param.type;   // 1：单事项 2：多事项 3： 一件事
            if(param.type == '3') {  // 一件事
            ...
            that.pageParamObj.title = param.name;
            that.jointStatus = param.jointStatus;
            // 根据事项获取材料
            that.getMatterial();
        }
    },
