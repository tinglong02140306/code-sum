        ...
        <div class="main-wrap clearfix">
            ...
            <div class="right-wrap fr">
                ...
                <el-table  :data="divisionData" @selection-change="divisionSelChange">
                    ...
                    <el-table-column class="font-max" width='120' class-name="el-tooltip" align="left" label="上级区划">
                        <template scope="scope" >
                    ...
                    <el-table-column class="font-max" align="center" label="区划类型">
                        <template scope="scope" >
                            ...
                            <el-select v-model="scope.row.xn" v-if="scope.row.status == '0'">
                                <el-option v-for="item in divisionTypeOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                            </el-select>
                        </template>
                    </el-table-column> 
    ...
    methods:{
            ...
            if(row.status == '1') {//编辑
                ...
                if (row.isNew == '1') {
                    addDto = {
                        ...
                        mc: row.mc,//区划名称
                        sjdm: row.sjdm,//上级区划
                        sjmc: that.$refs.selectTree.labelModel,
                ...
                if (res.flag) {
                    ...
                    setTimeout(function () {
                        if(option == 'addXzqh') {//新增
                            ...
                            // that.xzqhVal.push(data.dm);
                            // that.$refs.xzqhTree.xzqhFlag = that.xzqhVal;
                            // that.xzqhFlag = that.xzqhVal;
                            // that.getXzqhData();
        ...
        */
        getXzqhTreeData(treeType) {
            let that = this;
            that.xzqhVal = treeType == 'leftTree' ? that.xzqhVal:[];
            let obj = {
            ...
            unit.ajaxMerPost('/znsj-web/xzqh/getXzqhTreeData', obj, function (res) {
                if (res.flag == true) {
                    $.each(res.data, function (index, item) {
                        ...
                        that.divisionTree[0].highLight = false;
                        // res.data[that.i].highLight = true; 

                        if(that.sjdm == ""){
                            res.data[0].highLight = true; 
                        }else{
                            // let arr = JSON.stringify(that.sjdm);
                            // // let arr = that.sjdm;
                            // console.log(typeof(arr))
                            // arr.split('"');
                            // arr.splice(1,10,"0000000000")
                            // item.value == arr?item.highLight = true:item.highLight = false;
                            item.value == that.sjdm?item.highLight = true:item.highLight = false;
                        }
                    });
                    if(treeType == 'leftTree'){
                        that.sjdm = that.sjdm==""?res.data[0].value:that.sjdm;//默认第一个
                        that.selected = that.selected==""? res.data[0].label:that.selected;
    ...
    #division{
        ...
        .right-wrap{
            ...
            .el-table {
                ...
                margin-top: 10px;
                 // 超出截断
                .el-tooltip>.cell,.el-tooltip>.cell>span{
                    white-space: nowrap;
                    // min-width: 50px;
                    overflow: hidden;
                    text-overflow: ellipsis;
                }
                .el-tooltip>.cell>span{
                    display: block;
                    width: 100%;
                    overflow: hidden;
                    text-overflow: ellipsis;
