 ...
 * @Last Modified time: 2019-01-17 11:59:57
 * @description 导航面板
 */
<template>
...
<script>
import dataServerSetModel from "@/components/serverSet/dataServerSetModel";   // 数据服务配置
import functionServerSetModel from "@/components/serverSet/functionServerSetModel";   // 功能服务配置
    ...
    methods: {
        ...
        switchFun(index, flag) {
            ...
            if (!flag) {    //判断是否直接点击步骤条
                ...
                index = $index;
                if(that.step==0 && index == 1) {
                    for(let i = 0; i<that.$children.length; i++) {
                        that.$children[i].parentHandle && that.$children[i].parentHandle();
                    }
                } else if(index == 0) {
                    that.tabView = 'dataServerSetModel';
                    that.step = 0;
                }
            } else {
                setTimeout(function () {
                    ...
                    if (that.step == 0) {
                        ...
                        that.tabView = 'functionServerSetModel';
                    }
                }, 20);
            }
        },
