 ...
 * @Last Modified time: 2019-02-15 16:16:12
 * @description: 字典管理
 */
<template>
    <div id="container">
                ...
                <Form ref="formValidate2" :model="formValidate2" :rules="ruleValidate" :label-width="10">
                    <Row class="mr20 ml10">
                        ...
                        <FormItem label="" prop="" class="fr">
                            ...
                            <Button type="info" @click="showTreeHandle(null)">{{btnText}}</Button>
                        </FormItem>
                        </Col>
                    </Row>
                    <div v-show="!showTree" class="pr20 pl20">
                        <el-table :data="tableDate2.data" @selection-change="handleSelectionChange" tooltip-effect="light" :row-class-name="getRowClass" ref="parentTable" style="width: 100% !important" class="parent-table" :row-key="getRowKeys" :expand-row-keys="expands">
                            <!-- 展开项 -->
                                ...
                                <template scope="scope">
                                    <el-table :data="scope.row.children" tooltip-effect="light" @selection-change="handleSelectionChangeChild" :show-header="false" style="width: 100% !important" ref="childrenTable" class="children-table">
                                        ...
                                        <el-table-column label="数据名称">
                                            <template scope="scope">
                                                <span class="text-one cell el-tooltip item-text tip-title" :title="scope.row.dictName" href="javascript:void(0);">{{scope.row.dictName}}</span>
                                            </template>
                                        </el-table-column>
                                        ...
                                        <el-table-column align="center" label="操作" width="200">
                                            <template slot-scope="scope">
                                                ...
                                                <el-button title="编辑" type="text" @click="editValueHandle(scope.row)">编辑</el-button>
        ...
        <Modal id="dictionaryAddDialogModal" v-model="valueAddDialogModal" :title="valueTitle" :mask-closable="false" @on-visible-change="valueAddDialogVisibleChange">
            <Form :model="valueAddDialogFormItem" :rules="valueAddDialogRuleValidate" :label-width="80" ref="valueAddDialogRuleValidate">
                ...
                <FormItem label="上级名称" prop="parentName">
                    <Select v-model="valueAddDialogFormItem.sjxh" filterable clearable transfer ref="valueAddDialogRuleValidate1" :disabled="valueAddDialogFormItem.parentWithChildren">
                    ...
                    <el-table-column class="font-max" label="应用编码" prop="systemCode" align="left">
                        <template slot-scope="scope">
                            <el-form-item label-width="0" :prop="'manageData.'+scope.$index+'.systemCode'" :rules="rules.systemCode">
                                ...
                                <el-input v-else :class="{inputBorder: scope.row.status == '0'}" class="inputPadding" v-model="scope.row.systemCode" :title="scope.row.systemCode" maxlength="200" size="small" autosize placeholder="请输入应用编码"></el-input>
                            </el-form-item>
                            <!-- <span class="cell el-tooltip" :title="scope.row.systemCode" v-if="scope.row.status == '1'">{{scope.row.systemCode}}</span>
                    ...
                    <el-table-column class="font-max" label="应用名称" prop="systemName" align="left">
                        <template slot-scope="scope">
                            <el-form-item label-width="0" :prop="'manageData.'+scope.$index+'.systemName'" :rules="rules.systemName">
                                ...
                                <el-input v-else :class="{inputBorder: scope.row.status == '0'}" class="inputPadding" v-model="scope.row.systemName" :title="scope.row.systemName" maxlength="30" size="small" autosize placeholder="请输入应用名称"></el-input>
                            </el-form-item>
                            <!-- <span class="cell el-tooltip" :title="scope.row.systemName" v-if="scope.row.status == '1'">{{scope.row.systemName}}</span>
                    ...
                    <el-table-column class="font-max" label="应用密码" prop="systemPwd" align="left">
                        <template slot-scope="scope">
                            <el-form-item label-width="0" :prop="'manageData.'+scope.$index+'.systemPwd'" :rules="rules.systemPwd">
                                ...
                                <el-input v-else :class="{inputBorder: scope.row.status == '0'}" class="inputPadding" v-model="scope.row.systemPwd" :title="scope.row.systemPwd" maxlength="8" size="small" autosize placeholder="请输入应用密码"></el-input>
                            </el-form-item>
                            <!-- <span class="cell el-tooltip" :title="scope.row.systemPwd" v-if="scope.row.status == '1'">{{scope.row.systemPwd}}</span>
    ...
    methods: {
        ...
        typeAddDialogOk(name) {
            ...
            this.$refs[name].validate((valid) => {
                if (!valid) {
                    flag = false;
                    this.$Message.warning('数据校验失败,请检查输入!');
                }
            })
        ...
        valueAddBtn() {
            ...
            _that.valueAddDialogFormItem.parentWithChildren = false;
            _that.lastName = [];
        ...
        editValueHandle(row) {
            // parentWithChildren
            let _that = this,
                query = _that.$refs['valueAddDialogRuleValidate1'].$data.query;
            if (query) {
                this.$refs['valueAddDialogRuleValidate1'].$data.query = ''
            }
            ...
            _that.getLastName(row.dictId);
            setTimeout(function () {
                unit.ajaxMerPost('/bog-matter-mgr/dic/valueDetail', { 
                    id: row.dictId 
                }, function (res) {
                    if (res.flag) {
        ...
        valueAddDialogOk(name) {
            ...
            unit.ajaxPost('/bog-matter-mgr/dic/valueSave', _that.valueAddDialogFormItem).then(function (res) {
                ...
                if(!_that.valueAddDialogFormItem.dictId && !_that.showTree) {
                    _that.showTreeHandle(true);
                } else {
                    _that.showTreeHandle(_that.valueAddDialogFormItem.dictId);
                }
            });
        },
        ...
        valueDeleteDialogOk(deleId) {
            ...
            unit.ajaxMerPost(url, { ids: deleId }, function (res) {
                if (res.flag) {
                    _that.$Message.success('删除成功');
                    _that.handleSubmit2('formValidate2');
                    _that.valueSelectItem = [];
                    _that.valueSelectItemChild = [];
                } else {
                    _that.$Message.warning(res.errMsg || '删除失败');
                }
        ...
        valueSort(index, num) {
            ...
            unit.ajaxMerPost('/bog-matter-mgr/dic/valueSort', { id: id, movePositon: num }, function (res) {
                if (res.flag) {
                    _that.handleSubmit2('formValidate2', res.data.split(',')[1]);
                } else {
        ...
        systemListForAll() {
            ...
            unit.ajaxMerPost('/bog-matter-mgr/dictSystem/systemListForAll', {}, function (res) {
                if (res.flag) {
                    var sysNameData = [{ value: "", label: "请选择系统名称" }], data = res.data;
        ...
        handleSubmit2(name, expandId) {
            ...
            if (!flag) return;
            if(expandId) {
                _that.formValidate2.expandId = expandId;
            } else {
                _that.formValidate2.expandId = '';
            }
            const loading = this.$loading({
                lock: true,
                text: '加载中',
            ...
            unit.ajaxMerPost('/bog-matter-mgr/dic/valueList', _that.formValidate2, function (res) {
                ...
                if (res.flag) {
                    ...
                    for(let i = 0; i< res.data.rows.length; i++) {
                        if(res.data.rows[i].expand) {
                            arr.push(res.data.rows[i].dictId);
                        }
                        // title
                         setTimeout(() => {
                            let tips = $('.tip-title');
...
#container {
    ...
    .ivu-tree .ivu-tree-children .ivu-tree-children {
...
.moveLeft {
    background: url(../../assets/images/common/left-icon.png) center center
        no-repeat;
}

.moveRight {
    background: url(../../assets/images/common/right-icon.png) center center
        no-repeat;
}
