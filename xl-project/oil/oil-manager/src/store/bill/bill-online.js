...
class BillOnlineStore {
    ...
    //列表查询
    @observable dataList = [];
    @action getOrderOnlineQuery(params) {
        this.params = params;
        if (!this.isShowLoading) {
            this.setIsShowLoading(true);
        }
        console.log(JSON.stringify(params));
        http.post('/website/bill-fix/order-query-online',params,response=>{
    ...
    //差调
    @action billFixOnline(out_order_no,order_no){
        // console.log(out_order_no)
        this.setIsShowLoading(true);
        http.post('/website/bill-fix/fix-online',{out_order_no:out_order_no,order_no:order_no},response=>{
            this.setIsShowLoading(false);
            message.info("差调成功");
