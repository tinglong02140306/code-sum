...
import './OnlineStationDialog.scss';
...
@observer
class PartnerStationDialog extends Component {
    ...
    componentDidMount() {
        this.props.BillPartnerStore.getCompanyQuery();
        ...
        this.props.BillPartnerStore.getStationQuery(params);
    }
    ...
    handleTableChange = (pagination) => {
        ...
        this.props.BillPartnerStore.setSearchPagination(pager);
    ...
    selectCompanyOption=()=>{
        const {companyList} = this.props.BillPartnerStore;
    ...
    onCancel=()=>{
        this.props.BillPartnerStore.setIsShowSearchDialog(false);
    }

    onOk=()=>{
        const {stationList,typeModal} = this.props.BillPartnerStore;
        const {selectedRowKeys} = this.state;
        const key = selectedRowKeys[0];
        if (typeModal === 3){
            this.props.BillPartnerStore.setSearchData(stationList[key]);
        } else {
            this.props.BillPartnerStore.setOperationData(stationList[key]);
        }
        this.props.BillPartnerStore.setIsShowSearchDialog(false);
    }

    render() {
        const {isShowSearchLoading,searchPagination,stationList,isShowSearchDialog} = this.props.BillPartnerStore;
...
export default PartnerStationDialog;
